...
import { May08Chart, May15Chart, May22Chart } from "../../database/chart/may_chart"
import { ChartComponent } from "../lib/ui_helper"
import { May8AllSong } from "../../database/raw/may_8"
import { May15AllSongs } from "../../database/raw/may_15"
import { May22AllSong } from "../../database/raw/may_22"
...
function Index(props) {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        // const res = await fetch("/api/all-song")
        // if (res.ok) {
        //   const data = await res.json()
        //   setVideos(data)
        // }
        // const newArray = await helper.chartMaker(May22AllSong, May15AllSongs)
        // setVideos(newArray)
      } catch (error) {
        console.log(error)
      }
  ...
  return (
    <>
      ...
      <div className="youtube-page_container">
        {/*<Button onClick={() => helper.exportCSVFile(videos, "may_22")}>Click me</Button>*/}
        <Container className="chart-header">
          <img src={acvypImage} alt="" />
          <div className="title">
            <h1>Vtuber/Vsinger Music Chart</h1>
            <h2>KING BAN LIFTED</h2>
            <h6>Per May 22, 2021</h6>
          </div>
          <div className="week-column">
            <div className="week">
              <div className="label">Week</div>
              <div className="week-int">4</div>
            </div>
        ...
        <Container className="chart-container">
          {May22Chart.slice(10, 20).map((value, index) => (
