...
import { May09AllSongs } from "../../database/raw/may_9"
import { May09Chart, May16Chart } from "../../database/chart/may2_chart"
import { May16AllSongs } from "../../database/raw/may_16"
...
function Index(props) {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        // const res = await fetch("/api/no-big-2")
        // if (res.ok) {
        //   const data = await res.json()
        //   setVideos(data)
        // }
        // const newArray = await helper.chartMaker(May16AllSongs, May09AllSongs)
        // setVideos(newArray)
      } catch (error) {
        console.log(error)
      }
      ...
      <div className="youtube-page_container">
        <Container className="chart-header">
          ...
          <div className="title">
            ...
            <h2>HOLOLIVE NIJISANJI NOT ALLOWED</h2>
            <h6>Per May 16, 2021</h6>
          </div>
          <div className="week-column">
            <div className="week">
              <div className="label">Week</div>
              <div className="week-int">3</div>
            </div>
        ...
        <Container className="chart-container">
          {May16Chart.slice(0, 10).map((value, index) => (
            <ChartComponent video={value} index={index} indexAdd={1} key={index} />
          ))}
        </Container>
